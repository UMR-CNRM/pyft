...
from pyft.util import isint  # pylint: disable=import-error
...
if __name__ == '__main__':
    ...
    gStatement.add_argument('--removePrints', action='append',
                            help="Remove print statements from the source code. The argument " +
                                 ...
                                 "have to be removed. It is '/'-separated path with each " +
                                 "element having the form 'module:<name of the module>', " +
                                 "'sub:<name of the subroutine>' or " +
                                 "'func:<name of the function>'. Use 'ALL' to suppress all " +
                                 "the call statements regardless where there are.")
    ...
    try:
        ...
        for arg in optList:
                ...
                for scopePath, exclude in args.removePHYEXUnusedLocalVariables:
                    pft.removePHYEXUnusedLocalVar(
                        ...
                        [item.strip()
                         for item in exclude.split(',')] if exclude != 'NONE' else None,
                        **simplify)
            if arg == '--addExplicitArrayBounds':
                pft.addExplicitArrayBounds()
            if arg == '--addArrayParentheses':
                pft.addArrayParentheses()
            if arg == '--modifyAutomaticArrays':
            ...
            if arg == '--addArgInTree':
                for scopePath, varName, declStmt, pos in args.addArgInTree:
                    pft.addArgInTree(scopePath, varName, declStmt,
                                     int(pos), args.stopScopes.split('#'),
                                     parser=args.parser, parserOptions=parserOptions,
                                     wrapH=args.wrapH)

            # Applications
            if arg == '--addStack':
                pft.addStack(args.addStack, args.stopScopes.split('#'),
                             parser=args.parser, parserOptions=parserOptions,
                             wrapH=args.wrapH)
            if arg == '--deleteDrHook':
            ...
            if arg == '--addDrHook':
                pft.addDrHook()
            if arg == '--deleteBudgetDDH':
                pft.deleteBudgetDDH(**simplify)
            if arg == '--deleteNonColumnCallsPHYEX':
                pft.deleteNonColumnCallsPHYEX(**simplify)
            if arg == '--addMPPDB_CHECKS':
                pft.addMPPDB_CHECKS()
            ...
            if arg == '--mnhExpandConcurrent':
                pft.removeArraySyntax(concurrent=True, everywhere=False)
            if arg == '--inlineContainedSubroutines':
                pft.inlineContainedSubroutines(**simplify)
            if arg == '--inlineContainedSubroutinesPHYEX':
                pft.inlineContainedSubroutinesPHYEX(**simplify)
            if arg == '--expandAllArrays':
                pft.removeArraySyntax()
            if arg == '--expandAllArraysConcurrent':
                pft.removeArraySyntax(concurrent=True)
            if arg == '--expandAllArraysPHYEX':
                pft.expandAllArraysPHYEX()
            if arg == '--expandAllArraysPHYEXConcurrent':
                pft.expandAllArraysPHYEX(concurrent=True)
            if arg == '--removeIJDim':
                pft.removeIJDim(args.stopScopes.split('#'),
                                parser=args.parser, parserOptions=parserOptions,
                                wrapH=args.wrapH, **simplify)
            if arg == '--shumanFUNCtoCALL':
                pft.shumanFUNCtoCALL()
            if arg == '--buildACCTypeHelpers':
                pft.buildACCTypeHelpers()
            if arg == '--mathFunctoBRFunc':
                pft.mathFunctoBRFunc()
            if arg == '--convertTypesInCompute':
                pft.convertTypesInCompute()

            # OpenACC
            if arg == '--addACCData':
                pft.addACCData()
            if arg == '--craybyPassDOCONCURRENT':
                pft.craybyPassDOCONCURRENT()
            if arg == '--removebyPassDOCONCURRENT':
                pft.removebyPassDOCONCURRENT()
            if arg == '--addACCRoutineSeq':
                pft.addACCRoutineSeq(args.stopScopes.split('#'))
            if arg == '--removeACC':
                pft.removeACC()
            ...
            if arg == '--lowerCase':
                pft.lowerCase()
            if arg == '--changeIfStatementsInIfConstructs':
                pft.changeIfStatementsInIfConstructs()
            if arg == '--indent':
                pft.indent()
            if arg == '--removeIndent':
                pft.indent(indentProgramunit=0, indentBranch=0)
            if arg == '--removeEmptyLines':
                pft.removeEmptyLines()
            if arg == '--removeComments':
                pft.removeComments()
            if arg == '--updateSpaces':
                pft.updateSpaces()
            if kwUpdateCnt is None and arg in ('--alignContinuation', '--addBeginContinuation',
                                               '--removeBeginContinuation',
                                               '--emoveALLContinuation'):
                # Test if kwUpdateCnt is None is done to execute the transformation only once
                kwUpdateCnt = dict(align=args.alignContinuation,
                                   addBegin=args.addBeginContinuation,
                                   removeBegin=args.removeBeginContinuation,
                                   removeALL=args.removeALLContinuation)
                pft.updateContinuation(**kwUpdateCnt)
            if arg == '--prettify':
                pft.indent()
                pft.upperCase()
                pft.removeEmptyLines()
            ...
            if arg == '--removePrints':
                for rp in args.removePrints:
                    pft.removePrints(None if rp == 'ALL' else rp, **simplify)
            if arg == '--setFalseIfStmt':
