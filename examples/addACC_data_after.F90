...
!
!$acc data present ( PZZ, &PDZZ, &PSFTH, &PSFRV, &PPABSM, &PRHODREF, &PUM, &
!$acc &              PVM, &PTKEM, &PTHM, &PRVM, &PTHLM, &PRTM, &PSVM, &
!$acc &              PTHL_UP, &PRT_UP, &PU_UP, &PV_UP, &PRV_UP, &PRC_UP, &PRI_UP, &
!$acc &              PTHV_UP, &PW_UP, &PFRAC_UP, &PFRAC_ICE_UP, &PRSAT_UP, &PSV_UP, &PEMF, &
!$acc &              PDETR, &PENTR, &PBUO_INTEG, &KKLCL, &KKETL, &KKCTL, &PDEPTH)
...
!$acc data present ( OTEST, &OTESTLCL, &PFRAC_ICE, &PRHODREF, &PPRE_MINUS_HALF, &PPRE_PLUS_HALF, &PZZ, &
!$acc &              PDZZ, &PTHVM, &PTHLM, &PRTM, &PW_UP2, &PTH_UP, &PTHL_UP, &
!$acc &              PRT_UP, &PLUP, &PRC_UP, &PRI_UP, &PTHV_UP, &PRSAT_UP, &PRC_MIX, &
!$acc &              PRI_MIX, &PENTR, &PDETR, &PENTR_CLD, &PDETR_CLD, &PBUO_INTEG_DRY, &PBUO_INTEG_CLD, &
!$acc &              PPART_DRY)
...
!
!
!$acc data present ( PFRAC_ICE, &PP, &PTHL, &PRT, &PRR, &PRS, &PRG, &
!$acc &              PRH, &PTH, &PRV, &PRL, &PRI, &PRSATW, &PRSATI, &
!$acc &              PBUF)
