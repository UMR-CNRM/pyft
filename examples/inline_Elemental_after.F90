...
ELSE
  ...
  !               --------------------------------------
  DO JL=1, KPROMA
  IF (LDCOMPUTE(JL)) THEN
GNEGT=ZT(JL)<CST%XTT .AND. ZVART(JL, IRV)>ICED%XRTMIN(1)
ELSE
GNEGT=.FALSE.
END IF
...
IF (GNEGT) THEN
 ZZW_1=ALOG(ZT(JL))
 ZUSW=EXP(CST%XALPW - CST%XBETAW/ZT(JL) - CST%XGAMW*ZZW_1)          ! es_w
 ZZW_1=EXP(CST%XALPI - CST%XBETAI/ZT(JL) - CST%XGAMI*ZZW_1)           ! es_i
END IF
  
ZSSI=0.
IF (GNEGT) THEN
  ZZW_1=MIN(PPRES(JL)/2., ZZW_1)             ! safety limitation
  ZSSI=ZVART(JL, IRV)*(PPRES(JL)-ZZW_1) / (CST%XEPSILO*ZZW_1) - 1.0
                                               ! Supersaturation over ice
  ZUSW=MIN(PPRES(JL)/2., ZUSW)            ! safety limitation
  ZUSW=(ZUSW/ZZW_1)*((PPRES(JL)-ZZW_1)/(PPRES(JL)-ZUSW)) - 1.0
...
IF(GNEGT) THEN
  IF(ZT(JL)<CST%XTT-5.0 .AND. ZSSI>0.0) THEN
    ZZW_1=ICEP%XNU20*EXP(ICEP%XALPHA2*ZSSI-ICEP%XBETA2)
  ELSEIF(ZT(JL)<=CST%XTT-2.0 .AND. ZT(JL)>=CST%XTT-5.0 .AND. ZSSI>0.0) THEN
    ZZW_1=MAX(ICEP%XNU20*EXP(-ICEP%XBETA2 ), &                                                                                       
                ICEP%XNU10*EXP(-ICEP%XBETA1*(ZT(JL)-CST%XTT))*(ZSSI/ZUSW)**ICEP%XALPHA1)
  ENDIF
ENDIF
IF (GNEGT) THEN
  ZZW_1=ZZW_1-PCIT(JL)
  ZZW_1=MIN(ZZW_1, 50.E3) ! limitation provisoire a 50 l^-1
END IF

PBU_INST(JL, IRVHENI_MR)=0.

IF (GNEGT) THEN
  PBU_INST(JL, IRVHENI_MR)=MAX(ZZW_1, 0.0)*ICEP%XMNU0/PRHODREF(JL)
  PBU_INST(JL, IRVHENI_MR)=MIN(ZVART(JL, IRV), PBU_INST(JL, IRVHENI_MR))
END IF
!Limitation due to 0 crossing of temperature
IF(PARAMI%LFEEDBACKT) THEN
  ZW=0.
  IF (GNEGT) THEN
    ZW=MIN(PBU_INST(JL, IRVHENI_MR), &
              MAX(0., (CST%XTT/PEXN(JL)-ZVART(JL, ITH))/PLSFACT(JL))) / &
              MAX(PBU_INST(JL, IRVHENI_MR), 1.E-20)
  END IF
  PBU_INST(JL, IRVHENI_MR)=PBU_INST(JL, IRVHENI_MR)*ZW
  ZZW_1=ZZW_1*ZW
ENDIF
IF (GNEGT) THEN
  PCIT(JL)=MAX(ZZW_1+PCIT(JL), PCIT(JL))
END IF
