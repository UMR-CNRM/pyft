...
DO JL=1, KSIZE
  ZRCRAUTC(JL)=ICEP%XCRIAUTC/PRHODREF(JL)
END DO! Autoconversion rc threshold
IF(HSUBG_AUCV_RC=='NONE') THEN
  !Cloud water is entirely in low or high part
 !$mnh_expand_where(JL=1:KSIZE)
  DO JL=1, KSIZE
    IF (PRCT(JL)>ZRCRAUTC(JL)) THEN
      PHLC_HCF(JL)=1.
      PHLC_LCF(JL)=0.
      PHLC_HRC(JL)=PRCT(JL)
      PHLC_LRC(JL)=0.
    ELSE IF (PRCT(JL)>ICED%XRTMIN(2)) THEN
      PHLC_HCF(JL)=0.
      PHLC_LCF(JL)=1.
      PHLC_HRC(JL)=0.
      PHLC_LRC(JL)=PRCT(JL)
...
ELSEIF(HSUBG_AUCV_RC=='CLFR') THEN
 ...
 !$mnh_expand_where(JL=1:KSIZE)
  DO JL=1, KSIZE
    IF (PCF(JL)>0. .AND. PRCT(JL)>ZRCRAUTC(JL)*PCF(JL)) THEN
      PHLC_HCF(JL)=PCF(JL)
      PHLC_LCF(JL)=0.
      PHLC_HRC(JL)=PRCT(JL)
      PHLC_LRC(JL)=0.
    ELSE IF (PCF(JL)>0. .AND. PRCT(JL)>ICED%XRTMIN(2)) THEN
      PHLC_HCF(JL)=0.
      PHLC_LCF(JL)=PCF(JL)
      PHLC_HRC(JL)=0.0
      PHLC_LRC(JL)=PRCT(JL)
...
ELSEIF(HSUBG_AUCV_RC=='PDF ') THEN
  ...
  IF(HSUBG_PR_PDF=='SIGM') THEN
   ...
   !$mnh_expand_where(JL=1:KSIZE)
    DO JL=1, KSIZE
      IF (PRCT(JL)>ZRCRAUTC(JL)+PSIGMA_RC(JL)) THEN
        PHLC_HCF(JL)=1.
        PHLC_LCF(JL)=0.
        PHLC_HRC(JL)=PRCT(JL)
        PHLC_LRC(JL)=0.
      ELSE IF (PRCT(JL)> (ZRCRAUTC(JL)-PSIGMA_RC(JL)) .AND. PRCT(JL)<=(ZRCRAUTC(JL)+PSIGMA_RC(JL))       ) THEN
      ...
      ELSE IF (PRCT(JL)>ICED%XRTMIN(2) .AND. PCF(JL)>0.) THEN
        PHLC_HCF(JL)=0.
        PHLC_LCF(JL)=PCF(JL)
        PHLC_HRC(JL)=0.
        PHLC_LRC(JL)=PRCT(JL)
  ...
  ELSEIF(HSUBG_PR_PDF=='HLCRECTPDF' .OR. HSUBG_PR_PDF=='HLCISOTRIPDF' .OR. &
   ...
   !$mnh_expand_where(JL=1:KSIZE)
    DO JL=1, KSIZE
      IF (PRCT(JL).GT.0. .AND. PCF(JL).GT.0.) THEN
    ...
    DO JL=1, KSIZE
      ...
      ELSE IF (PRCT(JL).GT.0. .AND. PCF(JL).GT.0. .AND. ZHLC_RCMAX(JL).LE.ZRCRAUTC(JL)) THEN
...
!$mnh_expand_where(JL=1:KSIZE)
  ...
  END DO! Autoconversion ri threshold
!$mnh_end_expand_where(JL=1:KSIZE)
IF(HSUBG_AUCV_RI=='NONE') THEN
 !$mnh_expand_where(JL=1:KSIZE)
  !Cloud water is entirely in low or high part
  DO JL=1, KSIZE
    IF (PRIT(JL)>ZCRIAUTI(JL)) THEN
      PHLI_HCF(JL)=1.
      PHLI_LCF(JL)=0.
      PHLI_HRI(JL)=PRIT(JL)
      PHLI_LRI(JL)=0.
    ELSE IF (PRIT(JL)>ICED%XRTMIN(4)) THEN
      PHLI_HCF(JL)=0.
      PHLI_LCF(JL)=1.
      PHLI_HRI(JL)=0.
      PHLI_LRI(JL)=PRIT(JL)
...
ELSEIF(HSUBG_AUCV_RI=='CLFR') THEN
 ...
 !$mnh_expand_where(JL=1:KSIZE)
  DO JL=1, KSIZE
    IF (PCF(JL)>0. .AND. PRIT(JL)>ZCRIAUTI(JL)*PCF(JL)) THEN
      PHLI_HCF(JL)=PCF(JL)
      PHLI_LCF(JL)=0.
      PHLI_HRI(JL)=PRIT(JL)
      PHLI_LRI(JL)=0.
    ELSE IF (PCF(JL)>0. .AND. PRIT(JL)>ICED%XRTMIN(4)) THEN
      PHLI_HCF(JL)=0.
      PHLI_LCF(JL)=PCF(JL)
      PHLI_HRI(JL)=0.0
      PHLI_LRI(JL)=PRIT(JL)
