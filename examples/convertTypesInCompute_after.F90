!#PYFT transfo: --convertTypesInCompute
...
LOGICAL :: LPARAMIOCND2
LOGICAL :: LPARAMISEDIM_AFTER
LOGICAL :: LPARAMIWARM
CHARACTER(LEN=LEN(PARAMI%CSUBG_RC_RR_ACCR)) :: CPARAMISUBG_RC_RR_ACCR
CHARACTER(LEN=LEN(PARAMI%CSUBG_RR_EVAP)) :: CPARAMISUBG_RR_EVAP
CHARACTER(LEN=LEN(PARAMI%CSUBG_AUCV_RC)) :: CPARAMISUBG_AUCV_RC
CHARACTER(LEN=LEN(PARAMI%CSUBG_PR_PDF)) :: CPARAMISUBG_PR_PDF
CHARACTER(LEN=LEN(PARAMI%CSUBG_AUCV_RI)) :: CPARAMISUBG_AUCV_RI
LOGICAL :: LPARAMIPACK_MICRO
INTEGER :: NPARAMIPROMICRO
LOGICAL :: LBUCONFBU_ENABLE
LOGICAL :: LPARAMIDEPOSC
REAL, DIMENSION(KRR) :: XICEDRTMIN1KRR
REAL :: XCSTCPD
REAL :: XCSTCPV
REAL :: XCSTCL
REAL :: XCSTCI
...
!
!-------------------------------------------------------------------------------
XCSTRHOLW=CST%XRHOLW
XPARAMIVDEPOSC=PARAMI%XVDEPOSC
NDKT=D%NKT
NDIJT=D%NIJT
XICEDRTMIN7=ICED%XRTMIN(7)
XICEDRTMIN6=ICED%XRTMIN(6)
XICEDRTMIN5=ICED%XRTMIN(5)
XICEDRTMIN4=ICED%XRTMIN(4)
XICEDRTMIN3=ICED%XRTMIN(3)
XICEDRTMIN2=ICED%XRTMIN(2)
XCSTLVTT=CST%XLVTT
XCSTTT=CST%XTT
XCSTLSTT=CST%XLSTT
XCSTCI=CST%XCI
...
XCSTCPV=CST%XCPV
XCSTCPD=CST%XCPD
XICEDRTMIN1KRR=ICED%XRTMIN(1:KRR)
LPARAMIDEPOSC=PARAMI%LDEPOSC
LBUCONFBU_ENABLE=BUCONF%LBU_ENABLE
NPARAMIPROMICRO=PARAMI%NPROMICRO
LPARAMIPACK_MICRO=PARAMI%LPACK_MICRO
CPARAMISUBG_AUCV_RI=PARAMI%CSUBG_AUCV_RI
CPARAMISUBG_PR_PDF=PARAMI%CSUBG_PR_PDF
CPARAMISUBG_AUCV_RC=PARAMI%CSUBG_AUCV_RC
CPARAMISUBG_RR_EVAP=PARAMI%CSUBG_RR_EVAP
CPARAMISUBG_RC_RR_ACCR=PARAMI%CSUBG_RC_RR_ACCR
LPARAMIWARM=PARAMI%LWARM
LPARAMISEDIM_AFTER=PARAMI%LSEDIM_AFTER
LPARAMIOCND2=PARAMI%LOCND2
...
DO JK = IKTB,IKTE
  DO JIJ = IIJB,IIJE
    ...
    ENDIF
    ZDEVIDE = XCSTCPD + XCSTCPV*PRVT(JIJ,JK) + XCSTCL*(PRCT(JIJ,JK)+PRRT(JIJ,JK)) + XCSTCI*ZRICE
    ZT(JIJ,JK) = PTHT(JIJ,JK) * PEXN(JIJ,JK)
    ZZ_LSFACT(JIJ,JK)=(XCSTLSTT+(XCSTCPV-XCSTCI)*(ZT(JIJ,JK)-XCSTTT)) / ZDEVIDE
    ZZ_LVFACT(JIJ,JK)=(XCSTLVTT+(XCSTCPV-XCSTCL)*(ZT(JIJ,JK)-XCSTTT)) / ZDEVIDE

    !LLMICRO
    IF (KRR==7) THEN
      LLMICRO(JIJ,JK)=PRCT(JIJ,JK)>XICEDRTMIN2 .OR. &
                      PRRT(JIJ,JK)>XICEDRTMIN3 .OR. &
                      PRIT(JIJ,JK)>XICEDRTMIN4 .OR. &
                      PRST(JIJ,JK)>XICEDRTMIN5 .OR. &
                      PRGT(JIJ,JK)>XICEDRTMIN6 .OR. &
                      PRHT(JIJ,JK)>XICEDRTMIN7
    ELSE
      LLMICRO(JIJ,JK)=PRCT(JIJ,JK)>XICEDRTMIN2 .OR. &
                      PRRT(JIJ,JK)>XICEDRTMIN3 .OR. &
                      PRIT(JIJ,JK)>XICEDRTMIN4 .OR. &
                      PRST(JIJ,JK)>XICEDRTMIN5 .OR. &
                      PRGT(JIJ,JK)>XICEDRTMIN6
    ENDIF
  ENDDO
ENDDO
!
...
!               -------------------------------------
!
IF(.NOT. LPARAMISEDIM_AFTER) THEN
...
IF (CPARAMISUBG_RC_RR_ACCR=='PRFR' .OR. CPARAMISUBG_RR_EVAP=='PRFR') THEN
  IF (CPARAMISUBG_AUCV_RC=='PDF ' .AND. CPARAMISUBG_PR_PDF=='SIGM') THEN
    DO JK = IKTB, IKTE                                                                                                                  
      DO JIJ=IIJB, IIJE
    ...
    ENDDO
  ENDIF
  IF (CPARAMISUBG_AUCV_RC=='ADJU' .OR. CPARAMISUBG_AUCV_RI=='ADJU') THEN
    DO JK = IKTB, IKTE                                                                                                                
      DO JIJ=IIJB, IIJE
...
IF(LPARAMIPACK_MICRO) THEN
  ...
  IF (NPARAMIPROMICRO > 0 .AND. ISIZE > 0) THEN
    ! Cache-blocking is active
    ! number of chunks :
    IGPBLKS = (ISIZE-1)/MIN(NPARAMIPROMICRO,ISIZE)+1
...
!               -------------------------------------
!
IF (LPARAMIDEPOSC) THEN !cloud water deposition on vegetation
...
!DEC$ IVDEP
  DO JIJ = IIJB, IIJE
    PINDEP(JIJ) = XPARAMIVDEPOSC * PRCT(JIJ, IKB) * PRHODREF(JIJ, IKB) / XCSTRHOLW
    PRCS(JIJ, IKB) = PRCS(JIJ, IKB) - XPARAMIVDEPOSC * PRCT(JIJ, IKB) / PDZZ(JIJ, IKB)
