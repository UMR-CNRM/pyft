!#PYFT transfo: --addStack MESONH --stopScopes sub:CONDENSATION
...
REAL, DIMENSION(:,:), POINTER, CONTIGUOUS :: ZRT
     REAL, DIMENSION(:,:), POINTER, CONTIGUOUS :: ZTLK
! work arrays for T_l and total water mixing ratio
            REAL, DIMENSION(:,:), POINTER, CONTIGUOUS :: ZL
! length scale
INTEGER   :: ITPL            ! top levels of troposphere
REAL      :: ZTMIN           ! minimum Temp. related to ITPL
!
REAL, DIMENSION(:,:), POINTER, CONTIGUOUS :: ZCPD
...
CALL MNH_MEM_POSITION_PIN('sub:CONDENSATION')
CALL MNH_MEM_GET(ZTLK, 1, D%NIJT, 1, D%NKT)
CALL MNH_MEM_GET(ZRT, 1, D%NIJT, 1, D%NKT)
CALL MNH_MEM_GET(ZL, 1, D%NIJT, 1, D%NKT)
CALL MNH_MEM_GET(ZLV, 1, D%NIJT, 1, D%NKT)
CALL MNH_MEM_GET(ZLS, 1, D%NIJT, 1, D%NKT)
CALL MNH_MEM_GET(ZCPD, 1, D%NIJT, 1, D%NKT)
JIJ=D%NIJB
!
!-------------------------------------------------------------------------------
!
...
END IF
!
CALL MNH_MEM_RELEASE('sub:CONDENSATION')
CONTAINS
