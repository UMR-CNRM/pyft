!#PYFT transfo: --prettify

MODULE MODE_ICE4_FAST_RH2
IMPLICIT NONE
CONTAINS
...
PARAMI%LPACK_INTERP,GWET(:),IBUF1(:),IBUF2(:),IBUF3(:),ZBUF1(:),ZBUF2(:),ZBUF3(:), IGWET, ICEP%XKER_SWETH(:,:),ZZW(:))
IF (IGWET > 0) THEN
!$mnh_expand_where(JL=1:KSIZE)
WHERE (GWET(1:KSIZE))
#ifdef REPRO48
PRH_TEND(1:KSIZE,IRSWETH) = ICEP%XFSWETH * ZZW(1:KSIZE)  * (PLBDAS(1:KSIZE) ** (ICED%XCXS - ICED%XBS)) * &
  (PLBDAH(1:KSIZE) ** ICED%XCXH)  * (PRHODREF(1:KSIZE) ** (- ICED%XCEXVT - 1.))  * (ICEP%XLBSWETH1 /&
    (PLBDAH(1:KSIZE) ** 2) +  ICEP%XLBSWETH2 / (PLBDAH(1:KSIZE) * PLBDAS(1:KSIZE)) +  ICEP%XLBSWETH3 / (PLBDAS(1:KSIZE) ** 2))
#else
PRH_TEND(1:KSIZE,IRSWETH) = ICEP%XFSWETH * ZZW(1:KSIZE)  * (PRST(1:KSIZE)) * (PLBDAH(1:KSIZE) ** ICED%XCXH)&
* (PRHODREF(1:KSIZE) ** (- ICED%XCEXVT))  * (ICEP%XLBSWETH1 / (PLBDAH(1:KSIZE) ** 2) +  ICEP%XLBSWETH2 / &
              (PLBDAH(1:KSIZE) * PLBDAS(1:KSIZE)) +  ICEP%XLBSWETH3 / (PLBDAS(1:KSIZE) ** 2))
#endif
...
PARAMI%LPACK_INTERP,GWET(:),IBUF1(:),IBUF2(:),IBUF3(:),ZBUF1(:),ZBUF2(:),ZBUF3(:), IGWET, ICEP%XKER_GWETH(:,:),ZZW(:))
IF (IGWET > 0) THEN
!$mnh_expand_where(JL=1:KSIZE)
WHERE (GWET(1:KSIZE))
PRH_TEND(1:KSIZE,IRGWETH) = ICEP%XFGWETH * ZZW(1:KSIZE)  * (PLBDAG(1:KSIZE) ** (ICED%XCXG - ICED%XBG)) * &
        (PLBDAH(1:KSIZE) ** ICED%XCXH)  * (PRHODREF(1:KSIZE) ** (- ICED%XCEXVT - 1.))  * (ICEP%XLBGWETH1 / &
        (PLBDAH(1:KSIZE) ** 2) +  ICEP%XLBGWETH2 / (PLBDAH(1:KSIZE) * PLBDAG(1:KSIZE)) +  ICEP%XLBGWETH3 / (PLBDAG(1:KSIZE) ** 2))
...
PARAMI%LPACK_INTERP,GWET(:),IBUF1(:),IBUF2(:),IBUF3(:),ZBUF1(:),ZBUF2(:),ZBUF3(:), IGWET, ICEP%XKER_RWETH(:,:),ZZW(:))
IF (IGWET > 0) THEN
!$mnh_expand_where(JL=1:KSIZE)
WHERE (GWET(1:KSIZE))
PRH_TEND(1:KSIZE,IRRWETH) = ICEP%XFRWETH * ZZW(1:KSIZE)  * (PLBDAR(1:KSIZE) ** (- 4)) * (PLBDAH(1:KSIZE) ** &
        ICED%XCXH)  * (PRHODREF(1:KSIZE) ** (- ICED%XCEXVT - 1.))  * (ICEP%XLBRWETH1 / (PLBDAH(1:KSIZE) ** 2) + &
ICEP%XLBRWETH2 / (PLBDAH(1:KSIZE) * PLBDAR(1:KSIZE)) +  ICEP%XLBRWETH3 / (PLBDAR(1:KSIZE) ** 2))
END WHERE
!$mnh_end_expand_where(JL=1:KSIZE)
END IF
END IF
...
END IF
PRH_TEND(JL,IFREEZ1) = PKA(JL) * (CST%XTT - PT(JL)) +  (PDV(JL) * (CST%XLVTT + (CST%XCPV - CST%XCL) * (PT(JL) - CST%XTT))  &
  * (CST%XESTT - PRH_TEND(JL,IFREEZ1)) / (CST%XRV * PT(JL)))
PRH_TEND(JL,IFREEZ1) = PRH_TEND(JL,IFREEZ1) * (ICEP%X0DEPH * PLBDAH(JL) ** ICEP%XEX0DEPH +  ICEP%X1DEPH &
   * PCJ(JL) * PLBDAH(JL) ** ICEP%XEX1DEPH) /  (PRHODREF(JL) * (CST%XLMTT - CST%XCL * (CST%XTT - PT(JL))))
PRH_TEND(JL,IFREEZ2) = (PRHODREF(JL) * (CST%XLMTT + (CST%XCI - CST%XCL) * (CST%XTT - PT(JL)))) /  (PRHODREF(JL) &
        * (CST%XLMTT - CST%XCL * (CST%XTT - PT(JL))))
END IF
ZRWETH_INIT(JL) = MAX(PRH_TEND(JL,IRIWETH) + PRH_TEND(JL,IRSWETH) + PRH_TEND(JL,IRGWETH), MAX(0.,PRH_TEND(JL,IFREEZ1) +&
...
END IF
LLDRYH(JL) = PT(JL) < CST%XTT .AND. ZRDRYH_INIT(JL) > 1.E-20 .AND.  MAX(0.,ZRWETH_INIT(JL) - PRH_TEND(JL,IRIWETH) -&
PRH_TEND(JL,IRSWETH)) >  MAX(0.,ZRDRYH_INIT(JL) - PRH_TEND(JL,IRIDRYH) - PRH_TEND(JL,IRSDRYH))
ELSE
...
END IF
PRHMLTR(JL) = PKA(JL) * (CST%XTT - PT(JL)) +  PDV(JL) * (CST%XLVTT + (CST%XCPV - CST%XCL) * (PT(JL) - CST%XTT))  *&
        (CST%XESTT - PRHMLTR(JL)) / (CST%XRV * PT(JL))
PRHMLTR(JL) = MAX(0.,(- PRHMLTR(JL) * (ICEP%X0DEPH * PLBDAH(JL) ** ICEP%XEX0DEPH +  ICEP%X1DEPH * PCJ(JL) * PLBDAH(JL) ** &
 ICEP%XEX1DEPH) -  (PRH_TEND(JL,IRCWETH) + PRH_TEND(JL,IRRWETH)) *  (PRHODREF(JL) * CST%XCL * (CST%XTT - PT(JL)))) &
