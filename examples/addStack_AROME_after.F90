...
REAL, DIMENSION(D%NIJT, D%NKT) :: ZRT; POINTER(IP_ZRT_, ZRT)
     REAL, DIMENSION(D%NIJT, D%NKT) :: ZTLK; POINTER(IP_ZTLK_, ZTLK)
! work arrays for T_l and total water mixing ratio
            REAL, DIMENSION(D%NIJT, D%NKT) :: ZL; POINTER(IP_ZL_, ZL)
! length scale
INTEGER   :: ITPL            ! top levels of troposphere
REAL      :: ZTMIN           ! minimum Temp. related to ITPL
!
REAL, DIMENSION(D%NIJT, D%NKT) :: ZCPD; POINTER(IP_ZCPD_, ZCPD)
REAL, DIMENSION(D%NIJT, D%NKT) :: ZLS; POINTER(IP_ZLS_, ZLS)

REAL, DIMENSION(D%NIJT, D%NKT) :: ZLV; POINTER(IP_ZLV_, ZLV)
...
YLSTACK=YDSTACK
IP_ZTLK_=YLSTACK%L(KIND(ZTLK)/4);YLSTACK%L(KIND(ZTLK)/4)=YLSTACK%L(KIND(ZTLK)/4)+KIND(ZTLK)*SIZE(ZTLK);IF(YLSTACK%L(KIND(ZTLK)/4)>YLSTACK%U(KIND(ZTLK)/4))CALL SOF('addStack_AROME_before.F90:ZTLK', KIND(ZTLK))
IP_ZRT_=YLSTACK%L(KIND(ZRT)/4);YLSTACK%L(KIND(ZRT)/4)=YLSTACK%L(KIND(ZRT)/4)+KIND(ZRT)*SIZE(ZRT);IF(YLSTACK%L(KIND(ZRT)/4)>YLSTACK%U(KIND(ZRT)/4))CALL SOF('addStack_AROME_before.F90:ZRT', KIND(ZRT))
IP_ZL_=YLSTACK%L(KIND(ZL)/4);YLSTACK%L(KIND(ZL)/4)=YLSTACK%L(KIND(ZL)/4)+KIND(ZL)*SIZE(ZL);IF(YLSTACK%L(KIND(ZL)/4)>YLSTACK%U(KIND(ZL)/4))CALL SOF('addStack_AROME_before.F90:ZL', KIND(ZL))
IP_ZLV_=YLSTACK%L(KIND(ZLV)/4);YLSTACK%L(KIND(ZLV)/4)=YLSTACK%L(KIND(ZLV)/4)+KIND(ZLV)*SIZE(ZLV);IF(YLSTACK%L(KIND(ZLV)/4)>YLSTACK%U(KIND(ZLV)/4))CALL SOF('addStack_AROME_before.F90:ZLV', KIND(ZLV))
IP_ZLS_=YLSTACK%L(KIND(ZLS)/4);YLSTACK%L(KIND(ZLS)/4)=YLSTACK%L(KIND(ZLS)/4)+KIND(ZLS)*SIZE(ZLS);IF(YLSTACK%L(KIND(ZLS)/4)>YLSTACK%U(KIND(ZLS)/4))CALL SOF('addStack_AROME_before.F90:ZLS', KIND(ZLS))
IP_ZCPD_=YLSTACK%L(KIND(ZCPD)/4);YLSTACK%L(KIND(ZCPD)/4)=YLSTACK%L(KIND(ZCPD)/4)+KIND(ZCPD)*SIZE(ZCPD);IF(YLSTACK%L(KIND(ZCPD)/4)>YLSTACK%U(KIND(ZCPD)/4))CALL SOF('addStack_AROME_before.F90:ZCPD', KIND(ZCPD))
JIJ=D%NIJB
!
!-------------------------------------------------------------------------------
!
