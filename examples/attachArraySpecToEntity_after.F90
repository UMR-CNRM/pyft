!#PYFT transfo: --attachArraySpecToEntity
...
!-----------------------------------------------------------------
      SUBROUTINE TURB1(CST,CSTURB,BUCONF,TURBN,NEBN,D,TLES,            &
...
CHARACTER(LEN=4),INTENT(IN):: HLBCX(2), HLBCY(2)  ! X- and Y-direc LBC
...
!
REAL,  INTENT(IN)   :: PDXX(D%NIJT,D%NKT),PDYY(D%NIJT,D%NKT),PDZZ(D%NIJT,D%NKT),PDZX(D%NIJT,D%NKT),PDZY(D%NIJT,D%NKT)
                                        ! metric coefficients
REAL,  INTENT(IN)   :: PZZ(D%NIJT,D%NKT)       !  physical distance
! between 2 succesive grid points along the K direction
REAL,    INTENT(IN)      ::  PDIRCOSXW(D%NIJT), PDIRCOSYW(D%NIJT), PDIRCOSZW(D%NIJT)
! Director Cosinus along x, y and z directions at surface w-point
REAL,    INTENT(IN)   ::  PCOSSLOPE(D%NIJT)       ! cosinus of the angle
                                 ! between i and the slope vector
REAL,    INTENT(IN)   ::  PSINSLOPE(D%NIJT)       ! sinus of the angle
                                 ! between i and the slope vector
REAL,    INTENT(IN)   ::  PZS(D%NIJT) ! orography (for LEONARD terms)
REAL,  INTENT(IN)      ::  PRHODJ(D%NIJT,D%NKT)    ! dry density * Grid size
REAL,  INTENT(IN)      ::  MFMOIST(D%NIJT,D%NKT) ! moist mass flux dual scheme
REAL,  INTENT(IN)      ::  PTHVREF(D%NIJT,D%NKT)   ! Virtual Potential
                                        ! Temperature of the reference state
REAL,    INTENT(IN) ::  PHGRAD(D%NIJT,D%NKT,KGRADIENTS)      ! horizontal gradients
!
REAL,    INTENT(IN)      ::  PSFTH(D%NIJT),PSFRV(D%NIJT),   &
! normal surface fluxes of theta and Rv
                                            PSFU(D%NIJT),PSFV(D%NIJT)
! normal surface fluxes of (u,v) parallel to the orography
REAL,  INTENT(IN)      ::  PSFSV(D%NIJT,KSV)
! normal surface fluxes of Scalar var.
!
!    prognostic variables at t- deltat
REAL,    INTENT(IN) ::  PPABST(D%NIJT,D%NKT)      ! Pressure at time t
REAL,    INTENT(IN) ::  PUT(D%NIJT,D%NKT),PVT(D%NIJT,D%NKT),PWT(D%NIJT,D%NKT) ! wind components
REAL,    INTENT(IN) ::  PTKET(D%NIJT,D%NKT)       ! TKE
REAL,  INTENT(IN) ::  PSVT(D%NIJT,D%NKT,KSV)        ! passive scal. var.
REAL,    INTENT(IN) ::  PSRCT(MERGE(D%NIJT,0,OCOMPUTE_SRC),&
                MERGE(D%NKT,0,OCOMPUTE_SRC))       ! Second-order flux
                      ! s'rc'/2Sigma_s2 at time t-1 multiplied by Lambda_3
REAL, INTENT(INOUT) :: PBL_DEPTH(D%NIJT)  ! BL height for TOMS
REAL, INTENT(INOUT) :: PSBL_DEPTH(D%NIJT) ! SBL depth for RMC01
!
!    variables for cloud mixing length
REAL, INTENT(IN)      ::  PCEI(MERGE(D%NIJT,0,KMODEL_CL==KMI .AND. HTURBLEN_CL/='NONE'),&
                MERGE(D%NKT,0,KMODEL_CL==KMI .AND. HTURBLEN_CL/='NONE'))
...
REAL,    INTENT(INOUT) ::  PTHLT(D%NIJT,D%NKT)       ! conservative pot. temp.
REAL,  INTENT(INOUT) ::  PRT(D%NIJT,D%NKT,KRR)         ! water var.  where
                             ! PRT(:,:,:,1) is the conservative mixing ratio
!
! sources of momentum, conservative potential temperature, Turb. Kin. Energy,
! TKE dissipation
REAL,    INTENT(INOUT) ::  PRUS(D%NIJT,D%NKT),PRVS(D%NIJT,D%NKT),PRWS(D%NIJT,D%NKT),PRTHLS(D%NIJT,D%NKT),PRTKES(D%NIJT,D%NKT)
! Source terms for all water kinds, PRRS(:,:,:,1) is used for the conservative
! mixing ratio
REAL,    INTENT(IN),OPTIONAL    ::  PRTKEMS(D%NIJT,D%NKT)
...
! saturation
REAL,  INTENT(OUT)     ::  PSIGS(MERGE(D%NIJT,0,OCOMPUTE_SRC),&
                MERGE(D%NKT,0,OCOMPUTE_SRC))
REAL,  INTENT(OUT),OPTIONAL     ::  PDRUS_TURB(D%NIJT,D%NKT)   ! evolution of rhoJ*U   by turbulence only
REAL,  INTENT(OUT),OPTIONAL     ::  PDRVS_TURB(D%NIJT,D%NKT)   ! evolution of rhoJ*V   by turbulence only
REAL,  INTENT(OUT),OPTIONAL     ::  PDRTHLS_TURB(D%NIJT,D%NKT) ! evolution of rhoJ*thl by turbulence only
REAL,  INTENT(OUT),OPTIONAL     ::  PDRRTS_TURB(D%NIJT,D%NKT)  ! evolution of rhoJ*rt  by turbulence only
REAL,  INTENT(OUT),OPTIONAL ::  PDRSVS_TURB(D%NIJT,D%NKT,KSV)  ! evolution of rhoJ*Sv  by turbulence only
REAL,  INTENT(IN)      ::  PFLXZTHVMF(D%NIJT,D%NKT)
!                                           MF contribution for vert. turb. transport
!                                           used in the buoy. prod. of TKE
REAL,  INTENT(OUT)  :: PWTH(D%NIJT,D%NKT)       ! heat flux
REAL,  INTENT(OUT)  :: PWRC(D%NIJT,D%NKT)       ! cloud water flux
REAL, INTENT(OUT) :: PWSV(D%NIJT,D%NKT,KSV)       ! scalar flux
REAL,  INTENT(OUT)  :: PTP(D%NIJT,D%NKT)        ! Thermal TKE production
                                                   ! MassFlux + turb
REAL,  INTENT(OUT),OPTIONAL  :: PTPMF(D%NIJT,D%NKT)      ! Thermal TKE production
                                                   ! MassFlux Only
REAL,  INTENT(OUT)  :: PDP(D%NIJT,D%NKT)        ! Dynamic TKE production
REAL,  INTENT(OUT)  :: PTDIFF(D%NIJT,D%NKT)     ! Diffusion TKE term
REAL,  INTENT(OUT)  :: PTDISS(D%NIJT,D%NKT)     ! Dissipation TKE term
!
TYPE(TBUDGETDATA),  INTENT(INOUT) :: TBUDGETS(KBUDGETS)
...
REAL,  INTENT(IN)    :: PLENGTHM(D%NIJT,D%NKT), PLENGTHH(D%NIJT,D%NKT)
!
REAL,  INTENT(OUT), OPTIONAL  :: PEDR(D%NIJT,D%NKT)  ! EDR
REAL,  INTENT(OUT), OPTIONAL  :: PLEM(D%NIJT,D%NKT)  ! Mixing length
REAL,   INTENT(OUT), OPTIONAL  ::  PTR(D%NIJT,D%NKT)          ! Transport prod. of TKE
REAL,   INTENT(OUT), OPTIONAL  ::  PDISS(D%NIJT,D%NKT)        ! Dissipation of TKE
REAL,  INTENT(INOUT), OPTIONAL  ::  PCURRENT_TKE_DISS(D%NIJT,D%NKT) ! if ODIAG_IN_RUN in mesonh
REAL,  INTENT(IN),OPTIONAL   ::  PSSTFL(D%NIJT)        ! Time evol Flux of T at sea surface (LOCEAN)
REAL,  INTENT(IN),OPTIONAL   ::  PSSTFL_C(D%NIJT)  ! O-A interface flux for theta(LOCEAN and LCOUPLES)
REAL,  INTENT(IN),OPTIONAL   ::  PSSRFL_C(D%NIJT)  ! O-A interface flux for vapor (LOCEAN and LCOUPLES) 
REAL,  INTENT(IN),OPTIONAL   ::  PSSUFL_C(D%NIJT)        ! Time evol Flux of U at sea surface (LOCEAN)
REAL,  INTENT(IN),OPTIONAL   ::  PSSVFL_C(D%NIJT)  !
REAL,  INTENT(IN),OPTIONAL   ::  PSSUFL(D%NIJT)   
REAL,  INTENT(IN),OPTIONAL   ::  PSSVFL(D%NIJT)  !
!
REAL,  INTENT(OUT), OPTIONAL :: PIBM_XMUT(D%NIJT,D%NKT) ! IBM turbulent viscosity
REAL,  INTENT(IN), OPTIONAL  :: PIBM_LS(D%NIJT,D%NKT) ! IBM Level-set function
!
!
!-------------------------------------------------------------------------------
!
...
!
REAL  ::     &
          ZCP(D%NIJT,D%NKT),                        &  ! Cp at t-1
          ZEXN(D%NIJT,D%NKT),                       &  ! EXN at t-1
          ZT(D%NIJT,D%NKT),                         &  ! T at t-1
          ZLOCPEXNM(D%NIJT,D%NKT),                  &  ! Lv/Cp/EXNREF at t-1
          ZLM(D%NIJT,D%NKT),ZLMW(D%NIJT,D%NKT),                   &  ! Turbulent mixing length (+ work array)
          ZLEPS(D%NIJT,D%NKT),                      &  ! Dissipative length
          ZTRH(D%NIJT,D%NKT),                       &  !
          ZATHETA(D%NIJT,D%NKT),ZAMOIST(D%NIJT,D%NKT),            &  ! coefficients for s = f (Thetal,Rnp)
          ZCOEF_DISS(D%NIJT,D%NKT),                 &  ! 1/(Cph*Exner) for dissipative heating
          ZFRAC_ICE(D%NIJT,D%NKT),                  &  ! ri fraction of rc+ri
          ZMWTH(D%NIJT,D%NKT),ZMWR(D%NIJT,D%NKT),ZMTH2(D%NIJT,D%NKT),ZMR2(D%NIJT,D%NKT),ZMTHR(D%NIJT,D%NKT),&  ! 3rd order moments
          ZFWTH(D%NIJT,D%NKT),ZFWR(D%NIJT,D%NKT),ZFTH2(D%NIJT,D%NKT),ZFR2(D%NIJT,D%NKT),ZFTHR(D%NIJT,D%NKT),&  ! opposite of verticale derivate of 3rd order moments
          ZTHLM(D%NIJT,D%NKT),ZRTKEMS(D%NIJT,D%NKT),              &  ! initial potential temp; TKE advective source
          ZSHEAR(D%NIJT,D%NKT), ZDUDZ(D%NIJT,D%NKT), ZDVDZ(D%NIJT,D%NKT),       &  ! horizontal-wind vertical gradient
          ZLVOCPEXNM(D%NIJT,D%NKT),ZLSOCPEXNM(D%NIJT,D%NKT),      &  ! Lv/Cp/EXNREF and Ls/Cp/EXNREF at t-1
          ZATHETA_ICE(D%NIJT,D%NKT),ZAMOIST_ICE(D%NIJT,D%NKT),    &  ! coefficients for s = f (Thetal,Rnp)
          ZRVSAT(D%NIJT,D%NKT), ZDRVSATDT(D%NIJT,D%NKT),          &  ! local array for routine compute_function_thermo
          ZWORK1(D%NIJT,D%NKT),ZWORK2(D%NIJT,D%NKT),              &  ! working array syntax
          ZETHETA(D%NIJT,D%NKT),ZEMOIST(D%NIJT,D%NKT),            &  ! coef ETHETA and EMOIST (for DEAR routine)
          ZDTHLDZ(D%NIJT,D%NKT),ZDRTDZ(D%NIJT,D%NKT),             &  ! dtheta_l/dz, drt_dz used for computing the stablity criterion
          ZCOEF_AMPL(D%NIJT,D%NKT),                 &  ! Amplification coefficient of the mixing length
                                         ! when the instability criterium is verified (routine CLOUD_MODIF_LM)
          ZLM_CLOUD(D%NIJT,D%NKT)                      ! Turbulent mixing length in the clouds (routine CLOUD_MODIF_LM)
!
!
REAL  :: ZRM(D%NIJT,D%NKT,KRR) ! initial mixing ratio
REAL  ::  ZTAU11M(D%NIJT),ZTAU12M(D%NIJT),  &
                                                 ZTAU22M(D%NIJT),ZTAU33M(D%NIJT),  &
            ! tangential surface fluxes in the axes following the orography
                                                 ZUSLOPE(D%NIJT),ZVSLOPE(D%NIJT),  &
            ...
            ! orography (ZUSLOPE,ZVSLOPE) at the surface.
                                                 ZUSTAR(D%NIJT), ZLMO(D%NIJT),     &
                                                 ZRVM(D%NIJT), ZSFRV(D%NIJT),ZWORK2D(D%NIJT)
...
REAL,  INTENT(IN)    :: PT(D%NIJT,D%NKT),PEXN(D%NIJT,D%NKT),PCP(D%NIJT,D%NKT)
!
REAL,  INTENT(OUT)   :: PLOCPEXN(D%NIJT,D%NKT)
REAL,  INTENT(OUT)   :: PAMOIST(D%NIJT,D%NKT),PATHETA(D%NIJT,D%NKT)
!
!-------------------------------------------------------------------------------
!
  IF (LHOOK) THEN
