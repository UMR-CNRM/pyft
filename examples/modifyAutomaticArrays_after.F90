!#PYFT transfo: --modifyAutomaticArrays {type},DIMENSION({doubledotshape}),ALLOCATABLE::{name}#ALLOCATE({name}({shape}))#DEALLOCATE({name})
...
REAL,DIMENSION(:,:),ALLOCATABLE::ZBU_PACK
!
!For packing
INTEGER :: IMICRO ! Case r_x>0 locations
INTEGER :: JL, JV
REAL,DIMENSION(:),ALLOCATABLE::ZRREVAV
REAL,DIMENSION(:),ALLOCATABLE::ZRAINFR
REAL,DIMENSION(:),ALLOCATABLE::ZHLI_HRI
REAL,DIMENSION(:),ALLOCATABLE::ZHLI_HCF
REAL,DIMENSION(:),ALLOCATABLE::ZHLC_HRC
REAL,DIMENSION(:),ALLOCATABLE::ZHLC_HCF
REAL,DIMENSION(:),ALLOCATABLE::ZCF
REAL,DIMENSION(:),ALLOCATABLE::ZEXN
REAL,DIMENSION(:),ALLOCATABLE::ZPRES
REAL,DIMENSION(:),ALLOCATABLE::ZRHODREF

 REAL,DIMENSION(:),ALLOCATABLE::ZCIT
REAL,DIMENSION(:),ALLOCATABLE::ZSIGMA_RC
! Standard deviation of rc at time t

LOGICAL,DIMENSION(:),ALLOCATABLE::LLMICRO
!
!Output packed tendencies (for budgets only)

REAL,DIMENSION(:,:),ALLOCATABLE::ZBU_SUM
!
!For mixing-ratio-splitting
 REAL,DIMENSION(:,:),ALLOCATABLE::ZVART
!Packed variables
   REAL,DIMENSION(:,:),ALLOCATABLE::ZEXTPK
...
! Used to replace the COUNT and PACK intrinsics on variables
INTEGER,DIMENSION(:),ALLOCATABLE::I2TOT
 INTEGER,DIMENSION(:),ALLOCATABLE::I1TOT
! Used to replace the COUNT and PACK intrinsics
!
INTEGER :: IC, JMICRO
LOGICAL :: LLSIGMA_RC, LL_AUCV_ADJU
!
!-------------------------------------------------------------------------------
ALLOCATE(ZBU_PACK(KSIZE, IBUNUM-IBUNUM_EXTRA))
ALLOCATE(ZCIT(KPROMA))
ALLOCATE(ZRHODREF(KPROMA))
ALLOCATE(ZPRES(KPROMA))
ALLOCATE(ZEXN(KPROMA))
ALLOCATE(ZCF(KPROMA))
ALLOCATE(ZHLC_HCF(KPROMA))
ALLOCATE(ZHLC_HRC(KPROMA))
ALLOCATE(ZHLI_HCF(KPROMA))
ALLOCATE(ZHLI_HRI(KPROMA))
ALLOCATE(ZRAINFR(KPROMA))
ALLOCATE(ZRREVAV(KPROMA))
ALLOCATE(ZSIGMA_RC(KSIZE2))
ALLOCATE(LLMICRO(KPROMA))
ALLOCATE(ZBU_SUM(KPROMA, IBUNUM-IBUNUM_EXTRA))
ALLOCATE(ZVART(KPROMA, 0:7))
ALLOCATE(ZEXTPK(KSIZE2, 0:7))
ALLOCATE(ZLATHAM_IAGGS(MERGE(KPROMA,0,OELEC)))
ALLOCATE(I1(KPROMA))
ALLOCATE(I2(KPROMA))
ALLOCATE(I1TOT(KSIZE))
ALLOCATE(I2TOT(KSIZE))
