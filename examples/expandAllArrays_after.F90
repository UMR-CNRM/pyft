...
IF (TURBN%CTURBLEN=='BL89' .OR. TURBN%CTURBLEN=='RM17' .OR. TURBN%CTURBLEN=='HM21' .OR. TURBN%LRMC01) THEN
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZTHLM(JIJ, JK) = PTHLT(JIJ, JK)
    END DO
  END DO
  DO JRR=1, KRR
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZRM(JIJ, JK, JRR) = PRT(JIJ, JK, JRR)
      END DO
    END DO
  END DO
END IF
...
DO JSV=1, KSV
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZRSVS(JIJ, JK, JSV)=PRSVS(JIJ, JK, JSV)
    END DO
  END DO
END DO
!
...
IF (KRR > 0) THEN
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZCP(JIJ, JK) = ZCP(JIJ, JK) + CST%XCPV * PRT(JIJ, JK, 1)
    END DO
  END DO
END IF
...
!$mnh_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)  
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZCP(JIJ, JK)  = ZCP(JIJ, JK) + CST%XCL * PRT(JIJ, JK, JRR)
    END DO
  END DO
!$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
END DO
...
!$mnh_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZCP(JIJ, JK)  = ZCP(JIJ, JK)  + CST%XCI * PRT(JIJ, JK, JRR)
    END DO
  END DO
!$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
END DO
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    ZCOEF_DISS(JIJ, JK) = 1/(ZCP(JIJ, JK) * ZEXN(JIJ, JK))
  END DO
END DO
!$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
!
...
!
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZT(JIJ, JK) =  PTHLT(JIJ, JK) * ZEXN(JIJ, JK)
    END DO
  END DO
  !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
!
...
!
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        IF (PRT(JIJ, JK, 2)+PRT(JIJ, JK, 4)>0.0) THEN
          ZFRAC_ICE(JIJ, JK) = PRT(JIJ, JK, 4) / ( PRT(JIJ, JK, 2) &
                                              +PRT(JIJ, JK, 4) )
        END IF
      END DO
    END DO
...
!
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZLOCPEXNM(JIJ, JK) = (1.0-ZFRAC_ICE(JIJ, JK))*ZLVOCPEXNM(JIJ, JK) &
                               +ZFRAC_ICE(JIJ, JK) *ZLSOCPEXNM(JIJ, JK)
      END DO
    END DO
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZAMOIST(JIJ, JK) = (1.0-ZFRAC_ICE(JIJ, JK))*ZAMOIST(JIJ, JK) &
                             +ZFRAC_ICE(JIJ, JK) *ZAMOIST_ICE(JIJ, JK)
      END DO
    END DO
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZATHETA(JIJ, JK) = (1.0-ZFRAC_ICE(JIJ, JK))*ZATHETA(JIJ, JK) &
                             +ZFRAC_ICE(JIJ, JK) *ZATHETA_ICE(JIJ, JK)
      END DO
    END DO
    !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
  ELSE
...
IF ( KRRL >= 1 ) THEN
  IF ( KRRI >= 1 ) THEN
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PRT(JIJ, JK, 1)  = PRT(JIJ, JK, 1)  + PRT(JIJ, JK, 2)  & 
                                        + PRT(JIJ, JK, 4)
      END DO
    END DO
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PRRS(JIJ, JK, 1) = PRRS(JIJ, JK, 1) + PRRS(JIJ, JK, 2) & 
                                        + PRRS(JIJ, JK, 4)
      END DO
    END DO
    ! Theta_l at t
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PTHLT(JIJ, JK)  = PTHLT(JIJ, JK)  - ZLVOCPEXNM(JIJ, JK) &
                                        * PRT(JIJ, JK, 2) &
                                      - ZLSOCPEXNM(JIJ, JK) * PRT(JIJ, JK, 4)
      END DO
    END DO
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PRTHLS(JIJ, JK) = PRTHLS(JIJ, JK) - ZLVOCPEXNM(JIJ, JK) &
                                        * PRRS(JIJ, JK, 2) &
                                      - ZLSOCPEXNM(JIJ, JK) * PRRS(JIJ, JK, 4)
      END DO
    END DO
    !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
  ELSE
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PTHLT(JIJ, JK)  = PTHLT(JIJ, JK)  - ZLOCPEXNM(JIJ, JK) &
                                        * PRT(JIJ, JK, 2)
      END DO
    END DO
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PRTHLS(JIJ, JK) = PRTHLS(JIJ, JK) - ZLOCPEXNM(JIJ, JK) &
                                        * PRRS(JIJ, JK, 2)
      END DO
    END DO
    !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
...
IF(PRESENT(PDRUS_TURB)) THEN
  DO JK=1, D%NKT
    DO JIJ=1, D%NIJT
      PDRUS_TURB(JIJ, JK) = PRUS(JIJ, JK)
    END DO
  END DO
  DO JK=1, D%NKT
    DO JIJ=1, D%NIJT
      PDRVS_TURB(JIJ, JK) = PRVS(JIJ, JK)
    END DO
  END DO
  DO JK=1, D%NKT
    DO JIJ=1, D%NIJT
      PDRTHLS_TURB(JIJ, JK) = PRTHLS(JIJ, JK)
    END DO
  END DO
  DO JK=1, D%NKT
    DO JIJ=1, D%NIJT
      PDRRTS_TURB(JIJ, JK)  = PRRS(JIJ, JK, 1)
    END DO
  END DO
  DO JSV=1, KSV
    DO JK=1, D%NKT
      DO JIJ=1, D%NIJT
        PDRSVS_TURB(JIJ, JK, JSV)  = PRSVS(JIJ, JK, JSV)
      END DO
    END DO
  END DO
END IF
...
!           ------------------
  ...
  CASE ('RM17')
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZSHEAR(JIJ, JK) = SQRT(ZDUDZ(JIJ, JK)*ZDUDZ(JIJ, JK) &
                                        + ZDVDZ(JIJ, JK)*ZDVDZ(JIJ, JK))
      END DO
    END DO
    !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
    CALL BL89(D,CST,CSTURB,TURBN,PZZ,PDZZ,PTHVREF,ZTHLM,KRR,ZRM,PTKET,ZSHEAR,ZLM,OOCEAN)
...
!           --------------------------------------------------
  ...
  CASE ('HM21')
    ...
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        ZLM(JIJ, JK) = MIN(ZLM(JIJ, JK),TURBN%XCADAP*ZLMW(JIJ, JK))
      END DO
    END DO
    !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
!
...
!
  CASE ('BLKR')
   ...
   DO JK=IKTB,IKTE
     ...
     DO JIJ=IIJB, IIJE
       ZLM(JIJ, JK) = ( 0.5*(PZZ(JIJ, JK)+PZZ(JIJ, JK+IKL)) - &
       & PZZ(JIJ, IKA+JPVEXT_TURB*IKL) ) * PDIRCOSZW(JIJ)
     END DO
     DO JIJ=IIJB, IIJE
       ZLM(JIJ, JK) = ZALPHA  * ZLM(JIJ, JK) * ZL0 / ( ZL0 + ZALPHA*ZLM(JIJ, JK) )
     END DO
...
!
   !$mnh_expand_array(JIJ=IIJB:IIJE)
   DO JIJ=IIJB, IIJE
     ZLM(JIJ, IKTB-1) = ZLM(JIJ, IKTB)
   END DO
   DO JIJ=IIJB, IIJE
     ZLM(JIJ, IKTE+1) = ZLM(JIJ, IKTE)
   END DO
   !$mnh_end_expand_array(JIJ=IIJB:IIJE)
!
!
...
IF (TURBN%LHARAT) THEN
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZLEPS(JIJ, JK)=PLENGTHM(JIJ, JK)*(3.75**2.)
    END DO
  END DO
  !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
ELSE
...
IF (TURBN%CTURBLEN=='HM21') THEN
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      ZLEPS(JIJ, JK) = MIN(ZLEPS(JIJ, JK),ZLMW(JIJ, JK)*TURBN%XCADAP)
    END DO
  END DO
  !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
END IF
...
ELSE
  DO JIJ=IIJB, IIJE
    ZUSLOPE(JIJ)=PUT(JIJ, IKA)
  END DO
  DO JIJ=IIJB, IIJE
    ZVSLOPE(JIJ)=PVT(JIJ, IKA)
  END DO
END IF
IF (OOCEAN) THEN
  DO JIJ=IIJB, IIJE
    ZUSLOPE(JIJ)=PUT(JIJ, IKU-1)
  END DO
  DO JIJ=IIJB, IIJE
    ZVSLOPE(JIJ)=PVT(JIJ, IKU-1)
  END DO
END IF
!
!
...
!$mnh_expand_array(JIJ=IIJB:IIJE)
DO JIJ=IIJB, IIJE
  ZCDUEFF(JIJ) =-SQRT ( (PSFU(JIJ)**2 + PSFV(JIJ)**2) /               &
                      (CST%XMNH_TINY + ZUSLOPE(JIJ)**2 + ZVSLOPE(JIJ)**2 ) )
END DO
!$mnh_end_expand_array(JIJ=IIJB:IIJE)
!
...
ELSE
  !$mnh_expand_array(JIJ=IIJB:IIJE)        
  DO JIJ=IIJB, IIJE
    ZTAU11M(JIJ) =2./3.*(  (1.+ (PZZ(JIJ, IKB+IKL)-PZZ(JIJ, IKB))  &
                             /(PDZZ(JIJ, IKB+IKL)+PDZZ(JIJ, IKB))  &
                         )   *PTKET(JIJ, IKB)                   &
                       -0.5  *PTKET(JIJ, IKB+IKL)                 &
                      )
  END DO
  !$mnh_end_expand_array(JIJ=IIJB:IIJE)
END IF
...
END DO
DO JIJ=IIJB, IIJE
  ZTAU22M(JIJ) =ZTAU11M(JIJ)
END DO
DO JIJ=IIJB, IIJE
  ZTAU33M(JIJ) =ZTAU11M(JIJ)
END DO
!
...
DO JK=1, IKT
  DO JIJ=IIJB, IIJE
    PTP(JIJ, JK) = PTP(JIJ, JK) &
                                 + CST%XG / PTHVREF(JIJ, JK) * ZWORK1(JIJ, JK)
  END DO
END DO
!$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
...
IF(PRESENT(PTPMF))  THEN
  ...
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PTPMF(JIJ, JK)=CST%XG / PTHVREF(JIJ, JK) * ZWORK1(JIJ, JK)
    END DO
  END DO
  !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
END IF
...
!$mnh_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PDRUS_TURB(JIJ, JK)   = PRUS(JIJ, JK) - PDRUS_TURB(JIJ, JK)
    END DO
  END DO
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PDRVS_TURB(JIJ, JK)   = PRVS(JIJ, JK) - PDRVS_TURB(JIJ, JK)
    END DO
  END DO
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PDRTHLS_TURB(JIJ, JK) = PRTHLS(JIJ, JK) - PDRTHLS_TURB(JIJ, JK)
    END DO
  END DO
  DO JK=1, IKT
    DO JIJ=IIJB, IIJE
      PDRRTS_TURB(JIJ, JK)  = PRRS(JIJ, JK, 1) - PDRRTS_TURB(JIJ, JK)
    END DO
  END DO
  !$mnh_end_expand_array(JIJ=IIJB:IIJE,JK=1:IKT)
  ...
  DO JSV=1, KSV
    DO JK=1, IKT
      DO JIJ=IIJB, IIJE
        PDRSVS_TURB(JIJ, JK, JSV)  = PRSVS(JIJ, JK, JSV) - PDRSVS_TURB(JIJ, JK, JSV)
      END DO
    END DO
  END DO
...
!
  DO JK=1, D%NKT
    DO JIJ=1, D%NIJT
      ZLEPS(JIJ, JK) = 0.
    END DO
  END DO!ZLEPS is used as a work array (not used anymore)
